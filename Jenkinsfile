    ...
    stages { 
        ...
        stage('Filter'){
            steps{
                ...
                python3 /home/kaydeee329/CTG-Randoop/filter_result.py $(pwd)/err$(( ${BUILD_NUMBER#0} -1 )).md.json $(pwd)/err$BUILD_NUMBER.md.json 4 $(git rev-parse HEAD^1) $(git rev-parse HEAD) filtered_errors$BUILD_NUMBER.md
                '''
            }
        }
        ...
        stage('Result'){
            steps{
                sh script:'''
                cd $(git rev-parse --show-toplevel)/randoop
                git add filtered_errors$BUILD_NUMBER.md
